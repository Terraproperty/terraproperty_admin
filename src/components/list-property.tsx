...
const listPropertyFormSchema = z.object({
...
export interface ListPropertyFormProps {
  onSubmit: (values: ListPropertyFormValues) => Promise<void>;
  initialValues?: Partial<ListPropertyFormValues>;
}

export function ListPropertyForm({ onSubmit, initialValues }: ListPropertyFormProps) {
  ...
  // Ensure projectName is always set in the form state
  useEffect(() => {
  ...
  useEffect(() => {
    if (initialValues) {
      ...
      setImagePreviews((initialValues.images ?? []).map(img =>
        typeof img === 'string' && img.startsWith('blob:') ? img : img // keep as is, or you can add logic for server images
      ));
    }
  }, [initialValues]);

  const handleFormSubmit = async (values: ListPropertyFormValues) => {
  ...
  return (
    <Form {...form}>
      ...
      <form onSubmit={form.handleSubmit(handleFormSubmit)} className="space-y-8 bg-white dark:bg-gray-900 p-8 rounded-lg shadow-lg max-w-2xl mx-auto border border-gray-200 dark:border-gray-700">
        <h2 className="text-2xl font-bold mb-6 text-center text-gray-800 dark:text-gray-100">List Your Property</h2>
              ...
              <FormItem>
                ...
                <FormControl>
                  <select {...field} disabled={isSubmitting} className="w-full border rounded px-3 py-2">
                    ...
                    <option value="Completion Plots">Completion Plots</option>
